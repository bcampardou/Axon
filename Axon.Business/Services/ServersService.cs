...
namespace Axon.Business.Services
{
    public class ServersService : Service<Server, ServerDTO, ServerAdapter, IServersRepository>, IServersService
    {
        ...
        protected override bool _onAfterCreateOrUpdate(Server entity, ActionType action)
        {
            var result = base._onAfterCreateOrUpdate(entity, action);

            var teamRepository = _serviceProvider.GetRequiredService<IServerTeammatesRepository>();
